...
> 目前工具鏈會先把這些網路拓樸事實整理到 `tables/network.csv`，再由 auditor 轉成 schema hints。只有 `present=true` 且已有明確 `switch_port` 的 proven row，才會直接驅動 active `&switch0` port / `&xport` 等 topology enablement；若只是從 lane label 推到的候選 row，應保持 `present=inferred`，不可直接落地。像 `network-leds` 這類非純 topology 屬性，仍需要另外的獨立證據。**但 `GPHY` 的 lane-swap 屬性例外：`present=inferred` row 可以只拿來當 detector 的 candidate list，真正落成 active `&mdio_bus/xphy*:enet-phy-lane-swap` 與對應 `&ethphytop:xphy*-enabled` 的，仍必須來自後續 tracing detector 的獨立證據。** 若 block diagram 沒有可用文字層，工具會先做 OCR fallback；但只有在 OCR 與 CPU datasheet XPORT inventory 同時支持 WAN/SFP 拓樸時，才會把 `wan_10g` 補成 `port_wan@xpon_ae`，不會因此順便猜出 LAN `port_xgphyN`。

### B. 有某些功能時，還要補的資料

| 功能 | 為什麼需要額外資料 | 你要補哪些資訊 |
|---|---|---|
| **SFP / 光模組** | 現有 dual-SFP 板都要提供 `i2c-bus`、`los-gpio`、`mod-def0-gpio`、`tx-disable-gpio`，有些還多了 `tx-power-gpio`、`tx-power-down-gpio`、`rx-power-gpio`；對應 serdes 端還會綁 `trx`、`lane-enable`、`lane-select`、signal detect pinctrl 與支援速率模式。[^9][^10] | 每個 **已實際裝配** 的 SFP cage 都要提供：I2C bus、MOD_DEF0、LOS、TX_DISABLE、TX_POWER、RX_POWER、TX_POWER_DOWN、signal detect、lane enable/select、支援 1G/2.5G/5G/10G 哪些模式。**若 schematic 同頁可同時看到 SFP cage、`I2C Address: 0xA0/A2`、`SFP_SCL`、`SFP_SDA`，且 bus 端落到 `SDA_0` / `SCL`，工具可把 `wan_sfp:i2c-bus = <&i2c0>;` 升成 active。若無法證明第二組 cage/path 實際存在，就不能落地第二個 `serdes` / `lan_sfp`。** |
...
| **I2C 裝置 / expander / mux** | 同樣是 68575 板，不同板上 I2C 拓樸差很多：`968575DV` 類板在 `i2c0` 下掛 `pca9557`/`pca9555`/`ina236`，而 `968575SV_PVT1` 類板則在 `i2c0` 下掛 `pca9548` 再把 WAN SFP 走到 mux channel。[^8][^12] | 每顆 I2C device 的 bus、7-bit 地址、用途（boardid expander / WLAN expander / current monitor / mux / EEPROM / SFP EEPROM）。**第二組 I2C (`i2c1`) 不能因 ref DTS 有就假設存在；必須能證成第二組 bus/pin 線路，若對應 pin 已被證成他用，則應直接排除。** |
| **LED** | 現有板的 LED 不是只寫 label；還要定義 `serial-shifters-installed`、每顆 LED 的 `crossbar-output`、是否 `active_low`、是否是 `network-leds`、亮度、是否由 USB port trigger。[^8][^13] | 每個 LED 的名稱、對應埠/功能、是否走 serial LED、crossbar index 或 GPIO、active high/low、是否有 default trigger。**若板上是 74HC595 這類外部 LED controller，LED DTS 應以 controller datasheet + 電路設計為主；ref DTS 只能當命名/結構模板。** |
| **Power control / Wi-Fi GPIO hog** | `ext_pwr_ctrl` 與 `&gpioc` 很容易被 signal 名稱誤導：像 `USB_POWER` 的 SoC ball 名稱若直接抽數字，會變成假的 `gpioc 3/31`；而 Wi‑Fi 的 `RF_DISABLE_L` / `PEWAKE` 則應該落在 `&gpioc` 的 `gpio-hog`，不是 power block。 | 請提供真正的 **board power-enable** signal（`POWER_CONTROL`）與 Wi‑Fi `RF_DISABLE_L` / `PEWAKE` 對應 GPIO。工具只會用 `POWER_CONTROL` 生成 active `&ext_pwr_ctrl`；`RF_DISABLE_L` 會生成 `GPIO_ACTIVE_LOW + output-low` hog，`PEWAKE` 會生成 `GPIO_ACTIVE_HIGH + output-high` hog。 |
| **Buttons / Reset / WPS** | `buttons` 節點除了 GPIO/IRQ 外，還包含 reset 長按秒數與 WPS short/long period 等行為參數；而且不同板的按鍵 GPIO 可能完全不同。[^8][^14] | Reset/WPS/其他按鍵的 GPIO、IRQ edge。**button 一旦被硬體證成存在，可沿用 ref DTS 內對應 button 的 `press/hold/release` 軟體行為子節點；但 `linux,code` / `linux,press` / `linux,release` 這類 semantic property 不自動沿用，不存在的 button block 應刪除，不保留。** 若你要覆蓋 ref 預設行為，請另外提供長按秒數、factory reset / WPS policy。 |
...
| **USB** | 有些板把 `usb_ctrl` 設為 `xhci-enable` 並兩個埠都開，有些板則額外標註 `port1-disabled`；電源控制 pin 也要綁到 pinctrl。[^8][^9][^12] | 有幾個 USB 埠、哪個要 disable、power enable pin、是否要 LED trigger。**USB controller 可依 SoC capability + 板上 wiring 啟用，但每個 port 是否存在 / disable 仍要看實際 port population。若 schematic 只證明 `USB0` 有完整 VBUS / connector / superspeed path，而 `USB1` 沒有對應 VBUS 或 superspeed wiring，工具可把 `&usb_ctrl { port1-disabled; }` 升成 active；不能只因看到 `USB1_PWRON` / `USB1_DP` / `USB1_DM` 就假設第二個 port 已裝配。** |
